/*
  ...
  RELEASE NOTES; 
  - Added serial debug statements in order to troubleshoot reversal behavior at high speed turns
  - Added neutral calibration for one second
...
uint32_t ulProgramModeNeutralCalTime;
...
void setup()
{
  ...
  ulProgramModeNeutralCalTime = millis() + 1000;
  gMode = MODE_PROGRAM;
}
...
void loop()
{
  ...
  if (gMode == MODE_PROGRAM)
  {
    ...
    else
    {
      ...
      if (ulProgramModeNeutralCalTime < millis())
      {
        if (unThrottleIn > unThrottleCenter + 10 || unThrottleIn < unThrottleCenter - 10)
        {
          unThrottleCenter = unThrottleIn;
        }
        if (unSteeringIn > unSteeringCenter + 10 || unSteeringIn < unSteeringCenter - 10)
        {
  ...
  if (gMode == MODE_RUN)
  {
    ...
    if (bUpdateFlags & STEERING_FLAG)
    {
      ...
      // Debug Statements for Motor Drive
      Serial.print("L ");
  ...
  if ((gDirection != gOldDirection) || (gGear != gOldGear))
  {
    ...
    // Debug statements for direction
    Serial.print("Dir ");
